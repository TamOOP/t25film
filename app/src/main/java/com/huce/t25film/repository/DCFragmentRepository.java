...
public class DCFragmentRepository {
    private MutableLiveData<List<Film>> listfilmsLiveData = new MutableLiveData<>();
    private static DCFragmentRepository instance;
    private Retrofit retrofit;
    private FilmService filmService;

    // instance
    public static synchronized DCFragmentRepository getInstance(){
        if(instance == null){
            instance = new DCFragmentRepository();
        }
        return instance;
    }

    // build retrofit va tao user api khi instance
    public DCFragmentRepository(){
        if(retrofit == null){
    ...
    public MutableLiveData<List<Film>> getAllFilms(OnFilmListCallback onFilmListCallback){
        Call<List<Film>> _filmsCall = filmService.getListFilmsDC();
